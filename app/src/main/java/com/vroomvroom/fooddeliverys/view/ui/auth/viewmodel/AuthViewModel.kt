package com.vroomvroom.fooddeliverys.view.ui.auth.viewmodel
...
import com.vroomvroom.fooddeliverys.repository.local.UserPreferences
...
import com.vroomvroom.fooddeliverys.utils.SmsBroadcastReceiverListener
...
class AuthViewModel @Inject constructor(
    ...
    fun registerPhoneNumber(number: String) {
        _isOtpSent.postValue(Resource.Loading)
        viewModelScope.launch(Dispatchers.IO) {
           firebaseAuthRepository.generatePhoneOtp(number)
            { data ->
                when (data) {
                    is Resource.Success -> {
    ...
    fun verifyPhoneOtp(verifyId: String, otp: String) {
        _isVerified.postValue(Resource.Loading)
        viewModelScope.launch(Dispatchers.IO) {
            val response = userRepository.verifyOtp(verifyId, otp)
            { data ->
                when (data) {
                    is Resource.Success -> {
    ...
    fun register(locationEntity: LocationEntity, userEntity: UserEntity) {
        _isRegistered.postValue(Resource.Loading)
        viewModelScope.launch(Dispatchers.IO) {
           authRepository.register(locationEntity, userEntity){ result ->
               when (result) {
                   is Resource.Success -> {
                       _isRegistered.postValue(result)
                   }
                   is Resource.Error -> {
                       _isRegistered.postValue(result)
                   }
