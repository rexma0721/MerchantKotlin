...
import com.vroomvroom.fooddeliverys.data.model.user.Phone
import com.vroomvroom.fooddeliverys.data.model.user.UserEntity
import com.vroomvroom.fooddeliverys.databinding.FragmentRegisterBinding
...
class RegisterFragment : BaseFragment<FragmentRegisterBinding>(
    ...
    private fun observeToken() {
        authViewModel.token.observe(viewLifecycleOwner) { token ->
            if (token != null) {
                val userEntity: UserEntity = UserEntity(token,binding.registerNameInputEditText.text?.toString(), emailAddress, Phone());
                currentLocation?.let { authViewModel.register(it, userEntity) }
            }
        }
    }
                ...
                is Resource.Error -> {
                    ...
                    dialog.show(
                        ...
                        when (type) {
                            ClickType.POSITIVE -> {
                                loadingDialog.show(getString(R.string.loading))
                                val userEntity: UserEntity = UserEntity("", "", "", Phone());
                                currentLocation?.let { authViewModel.register(it, userEntity) }
                                dialog.dismiss()
                            }
