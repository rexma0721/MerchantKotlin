...
class MerchantRepositoryImpl @Inject constructor(
    ...
    override suspend fun getMerchants(
        ...
        try {
            ...
            if(searchTerm != null) {
                val te = mFirestore.collection("Category").whereEqualTo("name", searchTerm).get()
                val teresult = te.await().documents
                temp = if(teresult.size > 0) {
                    mFirestore.collection("Merchants").whereArrayContains("categories", searchTerm).get()
                } else {
                    mFirestore.collection("Merchants").orderBy("name").startAt(searchTerm)
                        .endAt(searchTerm + "\uf8ff").get()
                }
            }
            val documents = temp!!.await().documents
                for (document in documents) {
                    ...
                    val uid = if(FirebaseAuth.getInstance().currentUser != null) {FirebaseAuth.getInstance().currentUser!!.uid} else{null}
                    fmerchant.favorite = if(uid == null){false} else{results.contains(uid)}
                    ...
                    fmerchant.ratings = if(tresults.size == 0){0.0} else{sum.toDouble()/ tresults.size}
    ...
    override suspend fun getMerchant(id: String): Resource<Merchant>? {
        ...
        try {
            ...
            val uid = if(FirebaseAuth.getInstance().currentUser != null) {FirebaseAuth.getInstance().currentUser!!.uid} else{null}
            merchantDto.favorite = if(uid == null){false} else{fresults.contains(uid)}
            val tde = mFirestore.collection("Merchants").document(document.id).collection("reviews").get()
            val tresults = tde.await().documents
            var sum: Int = 0;
            for(result in tresults) {
            ...
            merchantDto.ratings = if(tresults.size == 0){0.0}else{sum.toDouble()/ tresults.size}
    ...
    override suspend fun getFavorites(): Resource<List<Merchant>>? {
        ...
        try {
            val temp = mFirestore.collection("Merchants").whereArrayContains("favorites", FirebaseAuth.getInstance().currentUser!!.uid).get()
    ...
    override suspend fun updateFavorite(id: String): Resource<Boolean> {
        ...
        try {
            ...
            if(tempresult.contains(uid)){
                mFirestore.collection("Merchants").document(id).update("favorites", FieldValue.arrayRemove(uid)).await()
            } else {
                mFirestore.collection("Merchants").document(id).update("favorites", FieldValue.arrayUnion(uid)).await()
            }
